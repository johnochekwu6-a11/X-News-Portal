...
function signupCreator(){
    ...
    if(users.find(u => u.email === email || u.username === username)){ alert('Email or Username already exists'); return; }
...
function loginUser(){
    ...
    // Admin check
    if(user.email === 'maduekefelix76@gmail.com'){
...
function displayArticles(){
    ...
    articles.filter(a => a.status === 'approved').forEach(a => {
        ...
        div.innerHTML = `<h3>${a.title}</h3>
                         <p>${a.content}</p>
                         <span>Author: ${a.author} | Reach: ${a.reach}</span>`;
...
function displayCreatorArticles(){
    ...
    articles.filter(a => a.author === loggedInUser).forEach(a => {
        let div = document.createElement('div');
        div.className = 'article-card';
        div.innerHTML = `<h3>${a.title} - [${a.status}]</h3>
                         <p>${a.content}</p>
...
function searchArticles(){
    ...
    articles.filter(a => a.status === 'approved' && a.author.toLowerCase().includes(q))
            .forEach(a => {
                let div = document.createElement('div');
                div.className = 'article-card';
                div.innerHTML = `<h3>${a.title}</h3>
...
// ======= ADMIN FUNCTIONS =======
function approveArticle(i){
    articles[i].status = 'approved';
    users.find(u=>u.username===articles[i].author).status='active';
    localStorage.setItem('articles', JSON.stringify(articles));
    localStorage.setItem('users', JSON.stringify(users));
    displayAdminSection('Pending');
    displayArticles();
}
function rejectArticle(i){
    articles[i].status = 'rejected';
    localStorage.setItem('articles', JSON.stringify(articles));
    displayAdminSection('Pending');
    displayCreatorArticles();
}
function setReach(i){
    let val = parseInt(document.getElementById('reach'+i).value);
    if(!isNaN(val)){
        articles[i].reach = val;
        localStorage.setItem('articles', JSON.stringify(articles));
        displayAdminSection('Pending');
        displayArticles();
    }
